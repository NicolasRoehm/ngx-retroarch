...
import { RetroarchConfig }   from '../../models/retroarch-config.model';
...
export class EmulatorComponent implements OnInit, OnDestroy
{
  ...
  private async loadAssetBundle() : Promise<void>
  {
    if (!this.romReady || !this.wasmReady)
      return console.warn(`Unable to load the asset bundle : ${!this.romReady ? 'ROM' : 'WASM'} is not ready yet!`);
  ...
  private async initCore() : Promise<void>
  {
    ...
    // NOTE Init config
    // TODO Load stored config
    this.playerConfig = new PlayerConfig();
    const mainConfig  = new RetroarchConfig();
    ...
    FS.writeFile('/home/web_user/retroarch/userdata/retroarch.cfg', mainConfig.asRetroarchConfig() + /*this.nulKeys +*/ this.playerConfig.asRetroarchConfig() /*+ this.extraConfig*/);

    // Looking for changed settings
    // const cfg     = FS.readFile('/home/web_user/retroarch/userdata/retroarch-core-options.cfg');
    ...
    //   this.playerConfig.input_player1_x = 'h';
    //   FS.writeFile('/home/web_user/retroarch/userdata/retroarch.cfg', mainConfig.asRetroarchConfig() + /*this.nulKeys +*/ this.playerConfig.asRetroarchConfig() /*+ this.extraConfig*/);
    //   wasmTable.get(108)(); // NOTE Refresh config
    //   // const config = 'input_player1_x = "h"\n';
    //   // FS.createPath('/', 'home/web_user/retroarch/userdata/config/Snes9x', true, true);
    //   // FS.writeFile('/home/web_user/retroarch/userdata/config/Snes9x/Snes9x.cfg', config);
    // }, 5000);

    // setTimeout(() => {
    // for (let i = 0; i < wasmTable.length; i++)
    //   if (typeof wasmTable.get(i) === 'function')
    //   {
    //     try
    //     {
    //       wasmTable.get(i)();
  ...
  private devLog(msg : string, background ?: string) : void
  {
    if (!this.development)
      return;
    console.log(`%c ${msg} `, `background: ${background ? background : '#7b37ff'}; color: #fff`);
  }
